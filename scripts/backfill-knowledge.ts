...
import { buildKnowledgeBackfillPlan } from "@/lib/knowledge/backfill";
...
const budgetFilter = projectId ? { projectId } : companyId ? { project: { companyId } } : undefined;
  ...
  const report = { dryRun, companyId: companyId ?? null, projectId: projectId ?? null, correlationId, candidates: { items: 0, resources: 0, apus: 0, prices: 0 }, created: { items: 0, resources: 0, apus: 0, prices: 0 }, skipped: { items: 0, resources: 0, apus: 0, prices: 0 }, errors: [] as Array<{ domain: string; id: string; message: string }> };
  ...
  for (const row of items) {
    ...
    try {
      ...
      if (existing) { report.skipped.items++; continue; }
      await prisma.canonicalItem.create({ data: { name: row.description.trim(), normalizedName, canonicalUnit: row.unit, scope: "COMPANY", companyId: row.budget.project.companyId, status: "OBSERVED" } }); report.created.items++;
  ...
  for (const row of resources) {
    ...
    try {
      ...
      const canonical = existing ?? await prisma.canonicalResource.create({ data: { name: candidate.name, normalizedName, category: "BACKFILL", canonicalUnit: candidate.canonicalUnit, scope: "COMPANY", companyId: candidate.companyId, status: "OBSERVED" }, select: { id: true } });
      ...
      if (row.priceObservedAt) {
        ...
        await prisma.priceObservation.upsert({ where: { idempotencyKey: priceKey }, create: { idempotencyKey: priceKey, resourceId: canonical.id, value: row.unitPrice, currency: row.currency, unit: row.unit, companyId: row.companyId, observedAt: row.priceObservedAt, scope: "COMPANY", confidence: "MEDIUM", status: "OBSERVED" }, update: {} });
  ...
  for (const row of apus) {
    ...
    try {
      ...
      await prisma.knowledgeApuVersion.upsert({ where: { idempotencyKey: `backfill:apu:${row.id}` }, create: { idempotencyKey: `backfill:apu:${row.id}`, apuId: row.id, versionNumber: 1, name: row.name, unit: row.unit, performance: row.performance, contentHash, scope: "PROJECT", companyId: tenant.companyId, projectId: row.budgetItem.budget.projectId, createdById: undefined, beforeSnapshot: null, afterSnapshot: snapshot, resources: { create: snapshot.resources.map((resource) => ({ resourceId: resource.resourceId, description: resource.description, unit: resource.unit, quantity: resource.quantity, unitPrice: resource.unitPrice, resourceType: resource.resourceType, sortOrder: resource.sortOrder })) } }, update: {} });
