...
import { calculateApuRows } from "@/lib/calculations/apu";
...
export function PartidaSimilarityGeneratorPageContent({
  ...
  const orderedIncludedInsumos = useMemo(() => orderInsumosByApuGroup(includedInsumos), [includedInsumos]);
  const bestScore = selectedSources.reduce((max, source) => Math.max(max, source.score), 0);
  const finalRows = useMemo(() => buildCalculatedGeneratedRows(orderedIncludedInsumos, performance), [orderedIncludedInsumos, performance]);
  const finalTotal = finalRows.reduce((sum, row) => sum + row.subtotal, 0);
  ...
  async function saveGeneratedPartida() {
    ...
    if (!orderedIncludedInsumos.length) {
      setError("Incluye al menos un insumo antes de guardar.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/partidas/similarity/save", {
        ...
        body: JSON.stringify({
          ...
          insumos: orderedIncludedInsumos.map((insumo) => ({
  ...
  function goNext() {
    ...
    if (currentStep === 2) {
      void aggregateInsumos();
      return;
    }

    if (currentStep === 3) {
      if (!suggestedInsumos.length) {
        setError("Agrega insumos antes de continuar.");
        return;
      }
      setError("");
      setCurrentStep(4);
    ...
    if (currentStep === 4) {
      void saveGeneratedPartida();
    }
  }

  const nextButtonLabel = currentStep === 1
    ? loadingState === "searching" ? "Buscando..." : "Siguiente"
    : currentStep === 2
      ? loadingState === "aggregating" ? "Agregando..." : "Siguiente"
      : currentStep === 4
        ? loadingState === "saving" ? "Guardando..." : "Guardar partida"
        : "Siguiente";

  return (
    <div className={mode === "embedded" ? "grid w-full min-w-0 gap-4" : "mx-auto grid w-full max-w-7xl gap-5"}>
      ...
      <StepSection number={2} title="Partidas candidatas">
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3">
            <div className="grid gap-3 lg:grid-cols-[220px_1fr]">
              <div>
                ...
                <Input value={manualFilter} onChange={(event) => setManualFilter(event.target.value)} placeholder="Buscar partida" className="mt-2" />
              </div>
              <div className="min-w-0 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)]">
                {manualMatches.length ? manualMatches.map((partida) => (
                  <button key={partida.id} type="button" onClick={() => addManualSource(partida)} className="grid w-full grid-cols-[1fr_70px] items-center gap-3 border-b border-[var(--app-border-soft)] px-3 py-2 text-left last:border-b-0 hover:bg-[var(--app-surface-hover)]">
                    <span className="truncate text-xs font-medium text-[var(--app-text-strong)]" title={partida.description}>{partida.description}</span>
                    <span className="text-right text-xs text-[var(--app-text-muted)]">{partida.unit}</span>
                  </button>
                )) : <p className="px-3 py-3 text-xs text-[var(--app-text-muted)]">Escribe para encontrar partidas adicionales.</p>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
                  ...
                  {(searchResult?.candidates ?? []).map((candidate) => {
                    ...
                    return (
                      <TR key={candidate.partida.id}>
                        ...
                        <TD className="truncate px-2 py-2 font-medium text-slate-900" title={candidate.partida.description}>{candidate.partida.description}</TD>
            ...
            <div className="grid gap-3 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 md:grid-cols-[1fr_120px_160px_160px]">
              ...
              <p className="flex items-center gap-2 text-xs text-[var(--app-text-muted)]">
                ...
                Revisa las fuentes y presiona Siguiente para agregar insumos.
              </p>
            </div>
          </div>
      ...
      {currentStep === 3 ? (
        <StepSection number={3} title="Insumos sugeridos">
          <div className="space-y-4">
            <GroupedInsumosTable insumos={suggestedInsumos} performance={performance} isExcelMode={isExcelMode} editable={false} onPatch={patchInsumo} onRemove={removeInsumo} />
          </div>
        </StepSection>
      ) : null}
      ...
      <StepSection number={4} title="Revision final / Vista final">
        <div className="min-w-0 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="max-w-xl text-xs text-slate-500">Nada se guarda hasta confirmar. La trazabilidad conserva fuentes, scores y calculos sugeridos.</p>
            <Button type="button" variant="outline" onClick={includeAllInsumos} disabled={!suggestedInsumos.length}>
              Incluir todos
            </Button>
          </div>

          <div className="min-w-0 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 text-xs">
            ...
            {resourceFilter.trim() ? (
              <div className="mt-3 grid gap-2">
                {resourceMatches.length ? resourceMatches.map((resource) => (
                  <button key={resource.id} type="button" onClick={() => addCatalogInsumo(resource)} className="grid min-w-0 gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-2 text-left hover:bg-[var(--app-surface-hover)] md:grid-cols-[1fr_70px_90px]">
                    <span className="truncate font-medium text-[var(--app-text-strong)]" title={resource.description}>{resource.description}</span>
                    <span className="text-[11px] text-[var(--app-text-muted)]">{resource.unit}</span>
                    <span className="text-right text-[11px] tabular-nums text-[var(--app-text)]">{formatCurrency(resource.unitPrice)}</span>
                  </button>
                )) : (
          ...
          <GroupedInsumosTable insumos={suggestedInsumos} performance={performance} isExcelMode={isExcelMode} editable onPatch={patchInsumo} onRemove={removeInsumo} />
          <ComparisonPanel
      ...
      <div className="sticky bottom-0 z-10 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--app-border)] bg-[var(--app-surface)]/95 px-1 py-3 backdrop-blur">
        <Button type="button" variant="outline" onClick={goBack} disabled={currentStep === 1 || loadingState !== "idle"}>
          Atras
        </Button>
        <Button type="button" onClick={goNext} disabled={loadingState !== "idle" || (currentStep === 4 && !selectedSources.length)} className="gap-2">
            {currentStep === 1 ? <Search className="h-4 w-4" /> : null}
            {currentStep === 4 ? <Check className="h-4 w-4" /> : null}
            {nextButtonLabel}
        </Button>
      </div>
    </div>
...
function GroupedInsumosTable({
  ...
  const calculatedRowsByKey = useMemo(() => {
    return new Map(buildCalculatedGeneratedRows(insumos, performance).map((row) => [row.id, row]));
  }, [insumos, performance]);
              ...
              <Table className="w-full table-fixed">
                ...
                <TBody>
                  {rows.length ? rows.map((insumo) => {
                    const calculatedRow = calculatedRowsByKey.get(insumo.key);
                    const displayUnitPrice = calculatedRow?.unitPrice ?? insumo.finalUnitPrice;
                    const displaySubtotal = calculatedRow?.subtotal ?? insumo.finalUnitPrice * insumo.finalQuantity;
                    const percentageBased = isPercentageBasedUnit(insumo.unit);

                    return (
                    <TR key={insumo.key} className={!insumo.included ? "opacity-55" : undefined}>
                      <TD className="truncate font-medium text-slate-900" title={insumo.description}>{insumo.description}</TD>
                      <TD className="tabular-nums">{Math.round(insumo.frequency * 100)}%</TD>
                      <TD>
                        {editable ? (
                          <Input type="number" step="0.0001" value={insumo.finalCrew ?? ""} onChange={(event) => onPatch(insumo.key, { finalCrew: event.target.value ? Number(event.target.value) : null })} className="w-full text-right tabular-nums" />
                        ) : (
                      ...
                      <TD>
                        {editable ? (
                          <Input type="number" step="0.0001" value={insumo.finalQuantity} onChange={(event) => onPatch(insumo.key, { finalQuantity: Number(event.target.value) })} className="w-full text-right tabular-nums" />
                        ) : (
                          <span className="tabular-nums">{insumo.suggestedQuantity}</span>
                        )}
                      </TD>
                      <TD className="truncate">{confidenceLabel(insumo.confidenceLevel)}</TD>
                      <TD>
                        {editable && !percentageBased ? (
                          <Input type="number" step="0.0001" value={insumo.finalUnitPrice} onChange={(event) => onPatch(insumo.key, { finalUnitPrice: Number(event.target.value) })} className="w-full text-right tabular-nums" />
                        ) : (
                          <span>{insumo.unitPrice == null && !percentageBased ? "Sin match" : formatCurrency(displayUnitPrice)}</span>
                        )}
                      </TD>
                      <TD className="truncate tabular-nums">{formatCurrency(displaySubtotal)}</TD>
                      {editable ? (
                        <TD>
...
function GroupedInsumosColGroup({ editable }: { editable: boolean }) {
  const widthClasses = editable
    ? ["w-[27%]", "w-[8%]", "w-[10%]", "w-[12%]", "w-[9%]", "w-[10%]", "w-[10%]", "w-[14%]"]
    : ["w-[35%]", "w-[9%]", "w-[10%]", "w-[14%]", "w-[10%]", "w-[11%]", "w-[11%]"];
...
function ComparisonColumn({
  ...
  return (
    <div className="min-w-0 space-y-3 overflow-hidden rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] p-3">
      <div className="flex items-start justify-between gap-3 border-b border-[var(--app-border-soft)] pb-3">
        <div className="min-w-0">
          <p className="truncate text-[11px] font-semibold uppercase text-[var(--app-text-muted)]">{title}</p>
          <p className="mt-1 truncate text-[10px] font-medium text-[var(--app-text-muted)]">Rendimiento: {performanceLabel}</p>
        </div>
        <p className="whitespace-nowrap text-xs font-semibold tabular-nums text-[var(--app-text-strong)]">{formatCurrency(total)}</p>
      </div>
      {APU_GROUPS.map((group) => {
              ...
              <Table className="w-full table-fixed text-[11px] leading-tight">
                ...
                <TBody>
                  {groupRows.length ? groupRows.map((row) => (
                    <TR key={row.id}>
                      <TD className="truncate px-2 py-1.5 font-medium text-[var(--app-text-strong)]" title={row.description}>{row.description}</TD>
                      <TD className="truncate px-2 py-1.5" title={row.unit}>{row.unit}</TD>
                      <TD className="truncate px-2 py-1.5 text-right tabular-nums">{formatOptionalNumber(row.crew ?? null)}</TD>
                      <TD className="truncate px-2 py-1.5 text-right tabular-nums">{formatCompactNumber(row.quantity)}</TD>
                      <TD className="truncate px-2 py-1.5 text-right tabular-nums">{formatCurrency(row.unitPrice)}</TD>
                      <TD className="truncate px-2 py-1.5 text-right tabular-nums">{formatCurrency(row.subtotal)}</TD>
                    </TR>
                  )) : (
                    <TR>
...
const WIZARD_STEPS = [
  { number: 1, label: "Fuente" },
  { number: 2, label: "Candidatas" },
  { number: 3, label: "Insumos" },
  { number: 4, label: "Revision" },
] as const;

function WizardStepIndicator({ currentStep }: { currentStep: number }) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] px-3 py-2">
      {WIZARD_STEPS.map((step) => {
        ...
        return (
          ...
          >
            <span className={cn("flex h-5 w-5 items-center justify-center rounded-full text-[11px]", active ? "bg-white/20" : completed ? "bg-emerald-100" : "bg-[var(--app-surface)]")}>
              {step.number}
            </span>
...
function orderInsumosByApuGroup(insumos: EditableSuggestedInsumo[]) {
  return insumos
    .map((insumo, index) => ({ insumo, index }))
    .sort((left, right) => {
      const leftGroupIndex = getApuGroupIndex(resolveApuGroup(left.insumo.resourceType));
      const rightGroupIndex = getApuGroupIndex(resolveApuGroup(right.insumo.resourceType));
...
function buildCalculatedGeneratedRows(insumos: EditableSuggestedInsumo[], performance: number): PartidaApuRowRecord[] {
  const rows = orderInsumosByApuGroup(insumos).map((insumo, index) => ({
    id: insumo.key,
  ...
  return calculateApuRows(rows, performance).sort((left, right) => left.sortOrder - right.sortOrder);
}

function getApuGroupIndex(groupKey: ApuGroupKey) {
  const index = APU_GROUPS.findIndex((group) => group.key === groupKey);
  return index === -1 ? APU_GROUPS.length : index;
}

function isPercentageBasedUnit(unit: string | null | undefined) {
  return (unit ?? "").trim().startsWith("%");
}
