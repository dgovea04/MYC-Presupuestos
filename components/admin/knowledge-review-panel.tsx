...
type Assertion = { id: string; subjectType: string; subjectId: string; predicate: string; scope: string; status: string; confidence: string; companyId: string | null; projectId: string | null; evidenceId: string | null; updatedAt: string };
...
type IntegrationError = { id: string; status: string; attemptCount: number; errorCode: string | null; errorMessage: string | null; findingId: string; decisionId: string };
...
export function KnowledgeReviewPanel({ items, resources, assertions = [], promotionCandidates = [], prices = [], yields = [], evidence = [], conflicts = [], integrationErrors = [] }: Props) {
  ...
  const filteredErrors = integrationErrors.filter((row) => matches(`${row.status} ${row.errorCode ?? ""} ${row.errorMessage ?? ""}`));

  async function confirmAlias(kind: "items" | "resources", id: string) {
    const value = alias[id]?.trim();
    if (!value) return setMessage("Escribe un alias antes de confirmarlo.");
  ...
  async function transition(row: Assertion, nextStatus: "CONFIRMED" | "REJECTED" | "DEPRECATED" | "CANONICAL") {
    ...
    if (nextStatus === "CANONICAL" && !window.confirm("La promoción GLOBAL requiere MFA y no modifica presupuestos. ¿Continuar?")) return;
    setBusy(row.id);
    try {
      const response = await fetch(`/api/admin/knowledge/assertions/${row.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ nextStatus, companyId: row.companyId, projectId: row.projectId ?? undefined, rejectionReason: reason, correlationId: crypto.randomUUID() }) });
      const payload = await response.json() as { error?: string };
      setMessage(response.ok ? `Assertion ${nextStatus.toLowerCase()} registrada.` : payload.error ?? "No se pudo actualizar la assertion.");
    } catch { setMessage("No se pudo conectar con la acción administrativa."); } finally { setBusy(null); }
  }
  ...
  async function retry(row: IntegrationError) {
    setBusy(row.id);
    try { const response = await fetch(`/api/admin/knowledge/jobs/${row.id}/retry`, { method: "POST" }); setMessage(response.ok ? "Retry solicitado." : "No se pudo solicitar el retry."); } finally { setBusy(null); }
  }

  const renderCandidates = (kind: "items" | "resources", rows: Candidate[]) => rows.map((row) => <div key={row.id} className="flex flex-col gap-2 border-t border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{row.name}</p><p className="text-xs text-slate-500">{row.scope} · {row.aliases.length} aliases</p></div><div className="flex gap-2"><input aria-label={`Alias para ${row.name}`} value={alias[row.id] ?? ""} onChange={(event) => setAlias((current) => ({ ...current, [row.id]: event.target.value }))} className="rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Alias confirmado" /><button type="button" onClick={() => void confirmAlias(kind, row.id)} className="rounded-xl bg-blue-600 px-3 py-2 text-sm font-medium text-white">Confirmar</button></div></div>);
  return <section className="space-y-5"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-semibold">Filtros administrativos</h2><div className="mt-3 flex flex-wrap gap-3"><input aria-label="Buscar en Knowledge" value={search} onChange={(event) => setSearch(event.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Buscar entidad, predicate o error" /><select aria-label="Filtrar por estado" value={status} onChange={(event) => setStatus(event.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm"><option value="ALL">Todos los estados</option>{["OBSERVED", "CONFIRMED", "VERIFIED", "CANONICAL", "REJECTED", "DEPRECATED", "OPEN", "RESOLVED", "DISMISSED"].map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Filtrar por scope" value={scope} onChange={(event) => setScope(event.target.value)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm"><option value="ALL">Todos los scopes</option><option value="PROJECT">PROJECT</option><option value="COMPANY">COMPANY</option><option value="GLOBAL">GLOBAL</option></select></div></div>
