...
export function AdminCloudAiSettings() {
  ...
  async function loadSettings() {
    ...
    try {
      ...
      setError(caughtError instanceof Error ? caughtError.message : "No se pudo cargar configuración del sistema.");
    } finally {
      setLoading(false);
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          isRecord(payload) && typeof payload.error === "string"
            ? payload.error
            : "Error al guardar configuración del sistema.",
        );
      }
      const data = readSystemSettings(payload);
      setSettings(data);
      ...
      setOpenaiTestResult("idle");
      setGeminiTestResult("idle");
      setSuccessMessage("Configuración del sistema guardada correctamente.");
      setTimeout(() => setSuccessMessage(""), 4000);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : "Error al guardar configuración del sistema.");
    } finally {
      setSaving(false);
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          isRecord(payload) && typeof payload.error === "string"
            ? payload.error
            : "Error al limpiar la key de OpenAI.",
        );
      }
      const data = readSystemSettings(payload);
      setSettings(data);
      ...
      setShowOpenaiKey(false);
      setOpenaiTestResult("idle");
      setSuccessMessage("API key de OpenAI eliminada del sistema.");
      setTimeout(() => setSuccessMessage(""), 4000);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : "No se pudo limpiar la key de OpenAI.");
    } finally {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          isRecord(payload) && typeof payload.error === "string"
            ? payload.error
            : "Error al limpiar la key de Gemini.",
        );
      }
      const data = readSystemSettings(payload);
      setSettings(data);
      ...
      setShowGeminiKey(false);
      setGeminiTestResult("idle");
      setSuccessMessage("API key de Gemini eliminada del sistema.");
      setTimeout(() => setSuccessMessage(""), 4000);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : "No se pudo limpiar la key de Gemini.");
    } finally {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <CardDescription>
                ...
                su propia key configurada en la sección de Proveedores Cloud IA personal.
              </CardDescription>
            </div>
          </div>
