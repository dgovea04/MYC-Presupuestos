...
import { History, RotateCcw, Save, Search, Upload, XCircle } from "lucide-react";
...
export function LocalResourcePriceAdminPanel({ canManage }: { canManage: boolean }) {
  ...
  async function load() {
    try {
      ...
      if (!resourceResponse.ok || !batchResponse.ok) throw new Error(resourcePayload.error ?? batchPayload.error ?? "No se pudo cargar el catálogo local.");
  ...
  useEffect(() => {
    if (canManage) void Promise.resolve().then(() => load());
  }, [canManage]);
  ...
  async function importWorkbook(file: File) {
    ...
    await runAction(async () => {
      ...
      setMessage("Excel importado como preview. Ningún precio fue publicado.");
      await load();
    });
  }
  ...
  async function publish(batch: LocalResourcePriceBatchSummary) {
    if (!window.confirm(`¿Publicar la versión ${batch.versionLabel}? Esta acción actualizará el catálogo global.`)) return;
    await runAction(async () => {
  ...
  async function reject(batch: LocalResourcePriceBatchSummary) {
    if (!window.confirm(`¿Rechazar la versión ${batch.versionLabel}? El preview quedará registrado sin publicar cambios.`)) return;
    await runAction(async () => {
  ...
  async function rollback(batch: LocalResourcePriceBatchSummary) {
    if (!window.confirm(`¿Revertir la versión ${batch.versionLabel}? Se creará una nueva versión de rollback.`)) return;
    await runAction(async () => {
    ...
    <section className="theme-surface-card rounded-2xl border p-6">
      ...
      {loading ? <p className="theme-muted-text mt-6 text-sm">Cargando catálogo local...</p> : (
        <div className="mt-6 space-y-6">
          {error ? <p className="theme-status-error rounded-xl border px-3 py-2 text-sm"><XCircle className="mr-2 inline h-4 w-4" />{error}</p> : null}
          ...
          <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
            <div className="space-y-3">
              ...
              <div className="theme-surface-card max-h-[32rem] overflow-auto rounded-2xl border">
                ...
                {visibleResources.length === 0 ? <p className="theme-muted-text px-4 py-8 text-center text-sm">No se encontraron insumos.</p> : visibleResources.map((resource) => <div key={resource.id} className="grid grid-cols-[1fr_8rem_7rem] items-center gap-3 border-b px-4 py-2.5 last:border-b-0"><div className="min-w-0"><p className="theme-strong-text truncate text-sm font-medium">{resource.code} · {resource.description}</p><p className="theme-muted-text text-xs">{resource.unit} · {resource.currency}</p></div><span className="theme-muted-text text-sm">{resource.unitPrice}</span><Input aria-label={`Nuevo precio para ${resource.code}`} inputMode="decimal" value={draftPrices[resource.id] ?? resource.unitPrice} onChange={(event) => setDraft(resource.id, event.target.value)} className="h-8 text-right text-sm" /></div>)}
              </div>
            </div>
            <div className="space-y-3">
              <h3 className="theme-strong-text font-semibold">Historial de versiones</h3>
              <div className="space-y-2">
                {batches.length === 0 ? <p className="theme-muted-text rounded-xl border border-dashed px-4 py-6 text-sm">Todavía no hay lotes locales.</p> : batches.map((batch) => <div key={batch.id} className="theme-muted-panel rounded-xl border p-3"><div className="flex items-start justify-between gap-3"><button type="button" className="text-left" onClick={() => void openBatch(batch.id)}><p className="theme-strong-text text-sm font-semibold">{batch.versionLabel}</p><p className="theme-muted-text text-xs">{sourceLabel(batch.source)} · {statusLabel(batch.status)}</p><p className="theme-muted-text mt-1 text-xs">{batch.changedRows} cambios · {batch.invalidRows} inválidas</p></button>{batch.status === "PREVIEW_READY" ? <div className="flex gap-1"><Button size="sm" variant="outline" onClick={() => void reject(batch)} disabled={busy}>Rechazar</Button><Button size="sm" onClick={() => void publish(batch)} disabled={busy}>Publicar</Button></div> : batch.status === "PUBLISHED" ? <Button size="sm" variant="outline" className="gap-1" onClick={() => void rollback(batch)} disabled={busy}><RotateCcw className="h-3 w-3" />Rollback</Button> : null}</div></div>)}
              </div>
            </div>
          </div>
