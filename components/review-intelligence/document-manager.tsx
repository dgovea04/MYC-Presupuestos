...
export function DocumentManager({ projectId, documents, selectedDocumentIds = [], onSelectionChange, onChanged }: { projectId: string; documents: ReviewDocumentView[]; selectedDocumentIds?: string[]; onSelectionChange?: (ids: string[]) => void; onChanged: () => void }) {
  ...
  async function upload(file: File) {
    ...
    try {
      const response = await fetch(`/api/projects/${encodeURIComponent(projectId)}/review-documents`, { method: "POST", body: formData });
  ...
  return (
    <Card className="theme-surface-card" data-testid="review-document-manager">
      ...
      <CardContent className="space-y-3">
        ...
        {documents.map((document) => <DocumentRow key={document.id} document={document} selected={selectedDocumentIds.includes(document.id)} onToggle={() => toggleDocument(document.id)} />)}
      </CardContent>
    </Card>
  );
}

function DocumentRow({ document, selected, onToggle }: { document: ReviewDocumentView; selected: boolean; onToggle: () => void }) {
