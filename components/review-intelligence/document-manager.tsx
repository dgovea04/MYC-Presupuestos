...
export function DocumentManager({ projectId, documents, selectedDocumentIds = [], selectedSheetNames = [], onSelectionChange, onSheetSelectionChange, onChanged }: { projectId: string; documents: ReviewDocumentView[]; selectedDocumentIds?: string[]; selectedSheetNames?: string[]; onSelectionChange?: (ids: string[]) => void; onSheetSelectionChange?: (names: string[]) => void; onChanged: () => void }) {
  ...
  async function reprocess(document: ReviewDocumentView, selection: { pages?: number[]; sheetNames?: string[] }) {
    const target = selection.pages?.[0] ?? selection.sheetNames?.[0] ?? "";
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof payload?.error === "string" ? payload.error : "No se pudo reprocesar la fuente.");
      ...
      onChanged();
    } catch (reprocessError) { setError(reprocessError instanceof Error ? reprocessError.message : "No se pudo reprocesar la fuente."); }
...
function DocumentRow({ document, selected, selectedSheetNames, onToggle, onToggleSheet, onClassified, onReplace, onDelete, onReprocess, reprocessing }: { document: ReviewDocumentView; selected: boolean; selectedSheetNames: string[]; onToggle: () => void; onToggleSheet: (sheetName: string) => void; onClassified: () => void; onReplace: () => void; onDelete: () => void; onReprocess: (selection: { pages?: number[]; sheetNames?: string[] }) => void; reprocessing: string | null }) {
