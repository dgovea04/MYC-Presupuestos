...
export function DocumentManager({ projectId, documents, selectedDocumentIds = [], selectedSheetNames = {}, onSelectionChange, onSheetSelectionChange, onChanged }: { projectId: string; documents: ReviewDocumentView[]; selectedDocumentIds?: string[]; selectedSheetNames?: Record<string, string[]>; onSelectionChange?: (ids: string[]) => void; onSheetSelectionChange?: (names: Record<string, string[]>) => void; onChanged: () => void }) {
  ...
  function toggleSheet(versionId: string, sheetName: string) {
    const current = selectedSheetNames[versionId] ?? [];
    onSheetSelectionChange?.({ ...selectedSheetNames, [versionId]: current.includes(sheetName) ? current.filter((value) => value !== sheetName) : [...current, sheetName] });
  }
...
function DocumentRow({ document, selected, selectedSheetNames, onToggle, onToggleSheet, onClassified, onReplace, onDelete }: { document: ReviewDocumentView; selected: boolean; selectedSheetNames: string[]; onToggle: () => void; onToggleSheet: (sheetName: string) => void; onClassified: () => void; onReplace: () => void; onDelete: () => void }) {
