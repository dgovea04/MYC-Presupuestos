...
export function ReviewIntelligencePage({ budgetId, projectId, initialRun, budgetName, projectName }: ReviewIntelligencePageProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadDocuments(), loadRuns()])
      .catch((loadError: unknown) => { if (active) setError(loadError instanceof Error ? loadError.message : "No se pudo cargar la revisiÃ³n."); })
  ...
  async function startReview() {
    if (selectedVersionIds.length === 0) { setError("Selecciona al menos un documento con versiÃ³n disponible."); return; }
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json().catch(() => null) as { error?: string } | null)?.error ?? "No se pudo iniciar la revisiÃ³n.");
      await loadRuns();
      runKey.current = null;
    } catch (startError) { setError(startError instanceof Error ? startError.message : "No se pudo iniciar la revisiÃ³n."); }
    finally { setStarting(false); }
  }

  return (
    <div className="space-y-5" data-testid="review-intelligence-page">
      <Card className="theme-surface-card"><CardHeader className="theme-surface-card-gradient"><PageHeaderCard icon={<ClipboardCheck className="h-5 w-5" />} title="RevisiÃ³n Inteligente" description={`Compara ${budgetName ?? "este presupuesto"} con evidencia de ${projectName ?? "tu proyecto"}, con trazabilidad y revisiÃ³n humana.`} badges={<><span className="rounded-full border border-slate-200 bg-white/70 px-2.5 py-1 text-xs font-medium">Presupuesto</span><span className="rounded-full border border-slate-200 bg-white/70 px-2.5 py-1 text-xs font-medium">Sin mutaciÃ³n automÃ¡tica</span></>} actions={<div className="flex flex-wrap gap-2"><Link href={`/budgets/${encodeURIComponent(budgetId)}`}><ActionButton action="open" label="Volver al presupuesto" variant="outline" /></Link><Button type="button" onClick={() => void startReview()} loading={starting} disabled={selectedVersionIds.length === 0}><Play className="h-4 w-4" aria-hidden="true" />Iniciar revisiÃ³n</Button></div>} /></CardHeader><CardContent className="space-y-3"><p className="text-sm text-[var(--app-text-muted)]">Selecciona fuentes PDF y XLSX, revisa el progreso persistido y decide cada hallazgo explÃ­citamente.</p>{error ? <p role="alert" className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{error}</p> : null}</CardContent></Card>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><div className="space-y-5"><DocumentManager projectId={projectId} documents={documents} selectedDocumentIds={selectedDocumentIds} onSelectionChange={setSelectedDocumentIds} onChanged={() => void loadDocuments()} />{runs.length > 0 ? <Card className="theme-surface-card"><CardHeader><CardTitle>Ejecuciones guardadas</CardTitle></CardHeader><CardContent className="space-y-2">{runs.map((run) => <button key={run.id} type="button" onClick={() => setSelectedRun(run)} className={run.id === selectedRun?.id ? "flex w-full items-center justify-between rounded-xl border border-sky-400 bg-sky-50 px-3 py-3 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500" : "flex w-full items-center justify-between rounded-xl border border-[var(--app-border)] px-3 py-3 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"}><span><strong className="block text-[var(--app-text-strong)]">{run.status}</strong><span className="text-xs text-[var(--app-text-muted)]">{new Date(run.createdAt).toLocaleString("es-PE")}</span></span><span className="text-xs text-[var(--app-text-muted)]">{run.progress.percent}%</span></button>)}</CardContent></Card> : null}<ReviewDashboard run={selectedRun} findingCount={findings.findings.length} documentCount={documents.length} /></div><div className="space-y-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-[var(--app-text-strong)]">Resultados</h2>{selectedRun ? <Button type="button" variant="ghost" size="sm" onClick={() => { void loadRuns(); void loadFindings(selectedRun); }}><RefreshCw className="h-4 w-4" aria-hidden="true" />Actualizar</Button> : null}</div>{loading && runs.length === 0 ? <Card><CardContent className="p-6 text-sm text-[var(--app-text-muted)]" role="status" aria-busy="true">Cargando fuentes y revisionesâ€¦</CardContent></Card> : null}{selectedRun ? <><FindingQueue data={findings} filters={filters} onFilterChange={setFilters} onOpenFinding={(id) => setSelectedFinding(findings.findings.find((item) => item.id === id))} /><FindingDetail finding={selectedFinding} canResolve runStatus={selectedRun.status} onChanged={() => { void loadFindings(selectedRun); }} /></> : <Card><CardContent className="p-6 text-sm text-[var(--app-text-muted)]">Cuando una revisiÃ³n termine, sus hallazgos aparecerÃ¡n aquÃ­.</CardContent></Card>}</div></div>
    </div>
  );
}

async function fetchJson<T>(url: string): Promise<T> { const response = await fetch(url); if (!response.ok) throw new Error("No se pudo cargar la informaciÃ³n de revisiÃ³n."); return await response.json() as T; }
