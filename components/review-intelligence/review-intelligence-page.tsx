...
export function ReviewIntelligencePage({ budgetId, projectId, initialRun, budgetName, projectName, canResolve = true }: ReviewIntelligencePageProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([loadDocuments(), loadRuns()])
      .catch((loadError: unknown) => { if (active) setError(loadError instanceof Error ? loadError.message : "No se pudo cargar la revisiÃƒÂ³n."); })
  ...
  async function startReview() {
    if (selectedVersionIds.length === 0) { setError("Selecciona al menos un documento con versiÃƒÂ³n disponible."); return; }
    ...
    try {
      ...
      if (!response.ok) throw new Error((await response.json().catch(() => null) as { error?: string } | null)?.error ?? "No se pudo iniciar la revisiÃƒÂ³n.");
      await loadRuns();
      runKey.current = null;
    } catch (startError) { setError(startError instanceof Error ? startError.message : "No se pudo iniciar la revisiÃƒÂ³n."); }
  ...
  return (
    <ReviewPermissionProvider canResolve={canResolve}><div className="space-y-5" data-testid="review-intelligence-page">
      <Card className="theme-surface-card"><CardHeader className="theme-surface-card-gradient"><PageHeaderCard icon={<ClipboardCheck className="h-5 w-5" />} title="RevisiÃƒÂ³n Inteligente" description={`Compara ${budgetName ?? "este presupuesto"} con evidencia de ${projectName ?? "tu proyecto"}, con trazabilidad y revisiÃƒÂ³n humana.`} badges={<><span className="rounded-full border border-slate-200 bg-white/70 px-2.5 py-1 text-xs font-medium">Presupuesto</span><span className="rounded-full border border-slate-200 bg-white/70 px-2.5 py-1 text-xs font-medium">Sin mutaciÃƒÂ³n automÃƒÂ¡tica</span></>} actions={<div className="flex flex-wrap gap-2"><Link href={`/budgets/${encodeURIComponent(budgetId)}`}><ActionButton action="open" label="Volver al presupuesto" variant="outline" /></Link><Button type="button" onClick={() => void startReview()} loading={starting} disabled={selectedVersionIds.length === 0}><Play className="h-4 w-4" aria-hidden="true" />Iniciar revisiÃƒÂ³n</Button></div>} /></CardHeader><CardContent className="space-y-3"><p className="text-sm text-[var(--app-text-muted)]">Selecciona fuentes PDF y XLSX, revisa el progreso persistido y decide cada hallazgo explÃƒÂ­citamente.</p>{error ? <p role="alert" className="rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{error}</p> : null}</CardContent></Card>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><div className="space-y-5"><DocumentManager projectId={projectId} documents={documents} selectedDocumentIds={selectedDocumentIds} onSelectionChange={setSelectedDocumentIds} onChanged={() => void loadDocuments()} />{runs.length > 0 ? <Card className="theme-surface-card"><CardHeader><CardTitle>Ejecuciones guardadas</CardTitle></CardHeader><CardContent className="space-y-2">{runs.map((run) => <button key={run.id} type="button" onClick={() => setSelectedRun(run)} className={run.id === selectedRun?.id ? "flex w-full items-center justify-between rounded-xl border border-sky-400 bg-sky-50 px-3 py-3 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500" : "flex w-full items-center justify-between rounded-xl border border-[var(--app-border)] px-3 py-3 text-left text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"}><span><strong className="block text-[var(--app-text-strong)]">{run.status}</strong><span className="text-xs text-[var(--app-text-muted)]">{new Date(run.createdAt).toLocaleString("es-PE")}</span></span><span className="text-xs text-[var(--app-text-muted)]">{run.progress.percent}%</span></button>)}</CardContent></Card> : null}<ReviewDashboard run={selectedRun} findingCount={findings.findings.length} documentCount={documents.length} /></div><div className="space-y-5"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-[var(--app-text-strong)]">Resultados</h2><div className="flex items-center gap-2">{selectedRun ? <span data-testid="review-lifecycle-status" className="rounded-full border border-slate-200 px-2 py-1 text-xs font-medium">{selectedRun.status}</span> : null}{selectedRun && canResolve && (selectedRun.status === "COMPLETED" || selectedRun.status === "COMPLETED_WITH_WARNINGS" || selectedRun.status === "UNDER_REVIEW") ? <Button type="button" variant="outline" size="sm" onClick={() => void markLifecycle()} loading={lifecycleUpdating}>{selectedRun.status === "UNDER_REVIEW" ? "Marcar como revisada" : "Pasar a revisión"}</Button> : null}{selectedRun ? <Button type="button" variant="ghost" size="sm" onClick={() => { void loadRuns(); void loadFindings(selectedRun); }}><RefreshCw className="h-4 w-4" aria-hidden="true" />Actualizar</Button> : null}</div></div>{loading && runs.length === 0 ? <Card><CardContent className="p-6 text-sm text-[var(--app-text-muted)]" role="status" aria-busy="true">Cargando fuentes y revisionesÃ¢â‚¬Â¦</CardContent></Card> : null}{selectedRun ? <><FindingQueue data={findings} filters={filters} onFilterChange={setFilters} onOpenFinding={(id) => setSelectedFinding(findings.findings.find((item) => item.id === id))} /><FindingDetail finding={selectedFinding} canResolve={canResolve} runStatus={selectedRun.status} onChanged={() => { void loadFindings(selectedRun); }} /></> : <Card><CardContent className="p-6 text-sm text-[var(--app-text-muted)]">Cuando una revisiÃƒÂ³n termine, sus hallazgos aparecerÃƒÂ¡n aquÃƒÂ­.</CardContent></Card>}</div></div>
    </div></ReviewPermissionProvider>
  );
}

async function fetchJson<T>(url: string): Promise<T> { const response = await fetch(url); if (!response.ok) throw new Error("No se pudo cargar la informaciÃƒÂ³n de revisiÃƒÂ³n."); return await response.json() as T; }
