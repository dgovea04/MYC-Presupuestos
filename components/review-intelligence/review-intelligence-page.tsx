...
import { ClipboardCheck, Download, Play, RefreshCw, Trash2 } from "lucide-react";
...
import { suggestDocumentClassification, type ClassificationSuggestion } from "@/lib/review-intelligence/classification";
import { reviewLabel, reviewStatusLabels } from "./labels";

export interface ReviewIntelligencePageProps { budgetId: string; projectId: string; companyId?: string; initialRun?: ReviewRunView; budgetName?: string; projectName?: string; canResolve?: boolean }

export function ReviewIntelligencePage({ budgetId, projectId, companyId, initialRun, budgetName, projectName, canResolve = true }: ReviewIntelligencePageProps) {
  ...
  async function loadFindings(run: ReviewRunView, nextFilters = filters) {
    ...
    if (nextFilters.assignedToId) params.set("assignedToId", nextFilters.assignedToId);
  ...
  async function startReview() {
    ...
    try {
      const response = await fetch(`/api/budgets/${encodeURIComponent(budgetId)}/review-runs`, {
        ...
        body: JSON.stringify({ documentVersionIds: selectedVersionIds, configuration: { maxFiles: 10, maxPdfPages: 300, maxFileSizeMb: 50, maxXlsxSheets: 20, tolerancePercent: "1.00", findingTypes: ["QUANTITY_MISMATCH", "UNIT_INCONSISTENCY", "TECHNICAL_SPEC_MISMATCH", "MISSING_DOCUMENTATION", "INCOMPLETE_APU"], ...(selectedSheetNames.length > 0 ? { xlsxSheetNames: selectedSheetNames } : {}) }, rulesVersion: "review-rules-v1" }),
      });
  ...
  return (
    <ReviewPermissionProvider canResolve={canResolve}><div className="min-w-0 max-w-full space-y-5 overflow-x-hidden" data-testid="review-intelligence-page">
      {selectedRun && runs.length > 1 ? <RunComparisonPanel budgetId={budgetId} selectedRun={selectedRun} runs={runs} /> : null}<div className="flex justify-end gap-2">{selectedRun ? <a href={`/api/budgets/${encodeURIComponent(budgetId)}/review-runs/${encodeURIComponent(selectedRun.id)}/export?format=csv`} download><Button type="button" variant="outline"><Download className="h-4 w-4" aria-hidden="true" />Exportar resumen</Button></a> : null}<Button type="button" variant="outline" className="border-rose-200 text-rose-700 hover:bg-rose-50" onClick={() => setClearHistoryDialogOpen(true)} loading={clearingHistory}><Trash2 className="h-4 w-4" aria-hidden="true" />Limpiar revisiones</Button></div>
...
function parseDocument(value: unknown): ReviewDocumentView[] { const row = record(value); if (!row || typeof row.id !== "string") return []; const version = record(row.currentVersion); const originalFileName = stringValue(row.originalFileName); const extractionWarnings = Array.isArray(version?.extractionWarnings) ? version.extractionWarnings.flatMap((warning) => parseWarning(warning).map((item) => item.message)) : []; const extractionCoverage = Array.isArray(version?.extractionCoverage) ? version.extractionCoverage.flatMap((entry) => { const coverageEntry = record(entry); return coverageEntry && typeof coverageEntry.coverage === "string" ? [{ page: typeof coverageEntry.page === "number" ? coverageEntry.page : undefined, worksheet: typeof coverageEntry.worksheet === "string" ? coverageEntry.worksheet : undefined, coverage: coverageEntry.coverage }] : []; }) : []; const coverage = extractionCoverage.flatMap((entry) => entry.worksheet ? [entry.worksheet] : []); const sheetNames = coverage.length > 0 ? coverage : Array.isArray(version?.sheetNames) ? version.sheetNames.filter((sheet): sheet is string => typeof sheet === "string") : []; const suggestion = parseClassificationSuggestion(row.classificationSuggestion) ?? suggestDocumentClassification({ fileName: originalFileName }); return [{ id: row.id, name: stringValue(row.name, originalFileName || "Documento"), originalFileName, category: stringValue(row.category, "OTHER"), status: stringValue(row.status, "UPLOADED"), classificationSuggestion: suggestion, currentVersion: version && typeof version.id === "string" ? { id: version.id, versionNumber: numberValue(version.versionNumber, 1), mimeType: stringValue(version.mimeType), fileSizeBytes: numberValue(version.fileSizeBytes), pageCount: typeof version.pageCount === "number" ? version.pageCount : null, sheetCount: typeof version.sheetCount === "number" ? version.sheetCount : null, extractionStatus: stringValue(version.extractionStatus), extractionWarnings, sheetNames, extractionCoverage } : null, warnings: [...(Array.isArray(row.warnings) ? row.warnings.filter((warning): warning is string => typeof warning === "string") : []), ...extractionWarnings] }]; }
function parseClassificationSuggestion(value: unknown): ClassificationSuggestion | undefined { const row = record(value); const categories = ["PLAN", "TECHNICAL_SPECIFICATION", "QUANTITY_TAKEOFF", "BUDGET", "APU", "OTHER"] as const; return row && typeof row.category === "string" && categories.includes(row.category as (typeof categories)[number]) && typeof row.score === "number" && Array.isArray(row.signals) ? { category: row.category as ClassificationSuggestion["category"], score: row.score, signals: row.signals.filter((signal): signal is string => typeof signal === "string") } : undefined; }
