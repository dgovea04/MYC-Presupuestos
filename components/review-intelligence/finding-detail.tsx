...
export function FindingDetail({ finding: selected, canResolve, onChanged, runStatus }: { finding?: FindingView; canResolve: boolean; onChanged: () => void; runStatus?: string }) {
  ...
  return <Card data-testid="finding-detail">
    ...
    <CardContent className="space-y-5">
      {finding.knowledge && finding.knowledge.length > 0 ? <section aria-label="Señales de MC Knowledge" className="space-y-2 rounded-xl border border-indigo-200 bg-indigo-50 p-4"><div><h4 className="text-sm font-semibold text-indigo-950">MC Knowledge influyó en este resultado</h4><p className="mt-1 text-xs text-indigo-800">Señales informativas; no reemplazan la evidencia documental primaria ni modifican el presupuesto.</p></div><ul className="space-y-2 text-xs text-indigo-950">{finding.knowledge.map((signal) => <li key={`${signal.id}-${signal.evidenceId ?? "none"}`} className="rounded-lg border border-indigo-200 bg-white/70 px-3 py-2"><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><span className="font-medium">{signal.id}</span><span>Scope: {signal.scope}</span><span>Confianza: {signal.confidence ?? "No disponible"}</span><span>Fecha: {signal.observedAt ? new Date(signal.observedAt).toLocaleDateString("es-PE") : "No disponible"}</span></div>{signal.evidenceId ? <a href={evidence?.viewUrl ?? `/api/admin/knowledge/evidence/${encodeURIComponent(signal.evidenceId)}`} target="_blank" rel="noreferrer" className="mt-1 inline-flex text-indigo-700 underline hover:text-indigo-950">Navegar a evidencia primaria</a> : <span className="mt-1 block text-indigo-700">Sin evidencia navegable</span>}</li>)}</ul></section> : null}
      <section aria-label="Visor estructurado de provenance" className="space-y-2">
        ...
        <div className="space-y-3 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 text-xs">
          ...
          <dl className="grid gap-2 sm:grid-cols-2"><Pair label="Texto original" value={evidence?.originalText ?? "—"} /><Pair label="Texto normalizado" value={evidence?.normalizedText ?? "—"} /><Pair label="Valor normalizado" value={evidence?.value ?? "No aplica: sin cantidad numérica"} /><Pair label="Unidad extraída" value={evidence?.unit ?? "—"} /><Pair label="Método" value={evidence?.extractionMethod ?? "—"} /></dl>
      ...
      {finding.findingType === "YIELD_MISMATCH" ? <section aria-label="Detalle de rendimiento" className="space-y-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-4"><div><h4 className="text-sm font-semibold text-[var(--app-text-strong)]">Detalle de rendimiento</h4><p className="mt-1 text-xs text-[var(--app-text-muted)]">Compara el rendimiento documentado con el registrado en el presupuesto.</p></div><dl className="grid gap-2 sm:grid-cols-2"><Pair label="Rendimiento documentado" value={finding.comparison?.documentValue ?? "—"} /><Pair label="Rendimiento presupuestado" value={finding.comparison?.budgetValue ?? "—"} /><Pair label="Diferencia de rendimiento" value={finding.comparison?.difference ?? "—"} /><Pair label="Unidad" value={finding.comparison?.unit ?? "—"} /></dl></section> : null}
      {finding.findingType === "TECHNICAL_SPEC_MISMATCH" ? <section aria-label="Detalle de especificación" className="space-y-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-4"><div><h4 className="text-sm font-semibold text-[var(--app-text-strong)]">Detalle de especificación</h4><p className="mt-1 text-xs text-[var(--app-text-muted)]">Revisa las especificaciones comparadas antes de tomar una decisión.</p></div><dl className="grid gap-2 sm:grid-cols-2"><Pair label="Especificación documentada" value={finding.comparison?.details?.documentSpecification ?? finding.comparison?.documentValue ?? "—"} /><Pair label="Especificación del presupuesto" value={finding.comparison?.details?.budgetSpecification ?? finding.comparison?.budgetValue ?? "—"} /></dl></section> : null}
      {finding.findingType === "INCOMPLETE_APU" ? <section aria-label="Detalle de componentes APU" className="space-y-2 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-4"><div><h4 className="text-sm font-semibold text-[var(--app-text-strong)]">Componentes APU faltantes</h4><p className="mt-1 text-xs text-[var(--app-text-muted)]">Componentes del APU presupuestado que no se identificaron en la evidencia.</p></div><dl><Pair label="Componentes faltantes" value={finding.comparison?.details?.missingComponents ?? "No se identificaron componentes faltantes."} /></dl></section> : null}
