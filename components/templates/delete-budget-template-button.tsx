  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(92vw,420px)] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_28px_80px_-34px_rgba(15,23,42,0.42)]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <Dialog.Title className="text-base font-semibold text-slate-950">Eliminar plantilla</Dialog.Title>
              <Dialog.Description className="mt-1 text-sm leading-5 text-slate-500">
                Esta accion elimina <span className="font-medium text-slate-700">{templateName}</span> de tu biblioteca. Los
