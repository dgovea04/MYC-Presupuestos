...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const membership = await assertWorkspaceMembership({ userId: session.user.id, companyId, minimumRole: "VIEWER" }); return NextResponse.json(await viewReviewEvidence({ evidenceId: (await params).id, companyId, userId: session.user.id, role: membership?.role ?? "VIEWER", correlationId: request.headers.get("X-Correlation-Id")?.trim() || randomUUID(), token: new URL(request.url).searchParams.get("token") ?? undefined })); } catch (error) { const message = error instanceof Error ? error.message : "No se pudo recuperar la evidencia"; return NextResponse.json({ error: message }, { status: /workspace|acceso|rol/i.test(message) ? 403 : /temporary|expired/i.test(message) ? 401 : 404 }); }
}
