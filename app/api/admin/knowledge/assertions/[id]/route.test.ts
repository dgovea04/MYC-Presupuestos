...
const { requireAdminSession, requireSuperAdminSession, transitionKnowledgeAssertion } = vi.hoisted(() => ({ requireAdminSession: vi.fn(), requireSuperAdminSession: vi.fn(), transitionKnowledgeAssertion: vi.fn() }));
vi.mock("@/lib/auth/session", () => ({ requireAdminSession, requireSuperAdminSession }));
...
describe("admin knowledge assertion route", () => {
  ...
  it("rejects unauthorized administrators", async () => {
    ...
    expect((await PATCH(new Request("http://localhost", { method: "PATCH", body: "{}" }), { params: Promise.resolve({ id: "a1" }) })).status).toBe(403);
  });
