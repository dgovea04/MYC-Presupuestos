...
const schema = z.object({ nextStatus: z.enum(["CONFIRMED", "VERIFIED", "CANONICAL", "REJECTED", "DEPRECATED"]), companyId: z.string().min(1), projectId: z.string().optional(), promotionScope: z.enum(["COMPANY", "GLOBAL"]).optional(), reviewDecisionId: z.string().min(1).optional(), rejectionReason: z.string().optional(), correlationId: z.string().min(1).max(200) }).strict();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await transitionKnowledgeAssertion({ assertionId: (await params).id, nextStatus: body.nextStatus, actorUserId: session.user.id, companyId: body.companyId, projectId: body.projectId, reviewDecisionId: body.reviewDecisionId, rejectionReason: body.rejectionReason, correlationId: body.correlationId, ...(body.promotionScope ? { promotionScope: body.promotionScope } : {}), ...(body.nextStatus === "CANONICAL" && (body.promotionScope ?? "GLOBAL") === "GLOBAL" ? { allowGlobalPromotion: true } : {}) });
