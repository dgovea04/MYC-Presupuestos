...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (companyId) {
      ...
      await assertKnowledgeWriteAccess({ actorUserId: session.user.id, entityType: "CanonicalItem", entityId: body.canonicalItemId, companyId, projectId: body.projectId, scope: body.scope === "GLOBAL" ? "GLOBAL" : undefined, capability: globalSession ? "knowledge.manage" : undefined, allowGlobalReference: body.scope !== "GLOBAL" });
    ...
    return knowledgeRouteErrorResponse(error, "No se pudo crear la observación");
  }
}
