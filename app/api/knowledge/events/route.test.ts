...
const { getAuthSession, requireSuperAdminSession, assertKnowledgeApiScopeAccess, recordKnowledgeEvent } = vi.hoisted(() => ({ getAuthSession: vi.fn(), requireSuperAdminSession: vi.fn(), assertKnowledgeApiScopeAccess: vi.fn(), recordKnowledgeEvent: vi.fn() }));
vi.mock("@/lib/auth/session", () => ({ getAuthSession, requireSuperAdminSession }));
vi.mock("@/lib/knowledge/api-access", () => ({ assertKnowledgeApiScopeAccess }));
vi.mock("@/lib/knowledge/events", () => ({ recordKnowledgeEvent }));

const body = { eventType: "ITEM_CONFIRMED", scope: "COMPANY", companyId: "c1", entityType: "CanonicalItem", entityId: "i1", sourceType: "HUMAN", idempotencyKey: "event-1" };
