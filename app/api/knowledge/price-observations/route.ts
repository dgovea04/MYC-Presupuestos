...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (companyId) {
      ...
      await assertKnowledgeWriteAccess({ actorUserId: session.user.id, entityType: "CanonicalResource", entityId: body.resourceId, companyId, projectId: body.projectId, scope: body.scope === "GLOBAL" ? "GLOBAL" : undefined, capability: globalSession ? "knowledge.manage" : undefined, allowGlobalReference: body.scope !== "GLOBAL" });
