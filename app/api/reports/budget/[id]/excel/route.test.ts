...
import { GET } from "@/app/api/reports/budget/[id]/excel/route";
...
describe("budget excel report route", () => {
  it("passes account and company metadata into the workbook export", async () => {
    ...
    vi.mocked(createBudgetWorkbook).mockResolvedValue(new Uint8Array([1, 2, 3]).buffer);

    await GET(new Request("http://localhost"), { params: Promise.resolve({ id: "budget-1" }) });

    expect(createBudgetWorkbook).toHaveBeenCalledWith(
