...
import { GET } from "@/app/api/reports/apu/[id]/excel/route";
...
describe("apu excel report route", () => {
  it("passes account and company metadata into the apu workbook export", async () => {
    ...
    vi.mocked(createApuWorkbook).mockResolvedValue(new Uint8Array([1, 2, 3]).buffer);

    await GET(new Request("http://localhost"), { params: Promise.resolve({ id: "budget-1" }) });

    expect(createApuWorkbook).toHaveBeenCalledWith(
