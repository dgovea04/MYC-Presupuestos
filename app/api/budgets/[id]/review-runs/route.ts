...
import { normalizeEvidenceMetadata } from "@/lib/review-intelligence/normalization";
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    const items = await prisma.budgetItem.findMany({ where: { budgetId: { in: [...budgetIds] } }, select: { id: true, budgetId: true, code: true, description: true, unit: true, quantity: true, unitPrice: true, discipline: true, apu: { select: { name: true, performance: true, resources: { select: { quantity: true, resourceType: true, resource: { select: { code: true, description: true } }, catalogPartida: { select: { description: true } } } } } } } });
    const reviewItems = items.map((item) => ({ ...item, discipline: item.discipline ?? undefined, technicalSpecification: item.apu?.name ?? undefined, yield: item.apu?.performance ? new Decimal(item.apu.performance.toString()) : undefined, apuComponents: item.apu?.resources.map((resource) => {
      const component = resource.resource?.description ?? resource.resource?.code ?? resource.catalogPartida?.description;
      return component ? `${resource.resourceType} | ${resource.quantity.toString()} | ${component}` : undefined;
    }).filter((value): value is string => value !== undefined) }));
    const selectedSheetNames = configuration.xlsxSheetNames;
    const input = { companyId: scope.budget.project.companyId, projectId: scope.budget.projectId, budgetId, budgetReference: { id: budgetId, companyId: scope.budget.project.companyId, projectId: scope.budget.projectId }, createdById: session.user.id, documentVersionIds: body.documentVersionIds, documentVersions: versions.map((version) => ({ ...version, extractionCoverage: normalizeExtractionCoverage(version.extractionCoverage) })), configuration, rulesVersion: body.rulesVersion, idempotencyKey, defer: true, budgetItems: reviewItems, evidence: evidence.filter((entry) => isSelectedWorksheetEvidence(entry.locationJson, selectedSheetNames)).map((entry) => { const metadata = typeof entry.metadataJson === "object" && entry.metadataJson !== null && !Array.isArray(entry.metadataJson) ? entry.metadataJson as Record<string, unknown> : {}; const normalizedMetadata = normalizeEvidenceMetadata(metadata); const components = Array.isArray(metadata.apuComponents) ? metadata.apuComponents.filter((value): value is string => typeof value === "string") : normalizedMetadata.apuComponents; return { id: entry.id, documentVersionId: entry.documentVersionId, originalText: entry.originalText, normalizedText: entry.normalizedText ?? undefined, sourceHash: entry.sourceHash, evidenceType: entry.evidenceType, confidence: entry.confidence, unit: normalizedMetadata.unit ?? entry.unit ?? undefined, quantity: normalizedMetadata.quantity ?? entry.value ?? undefined, unitPrice: normalizedMetadata.unitPrice, code: normalizedMetadata.code, description: normalizedMetadata.description, technicalSpecification: normalizedMetadata.technicalSpecification, discipline: normalizedMetadata.discipline, yield: normalizedMetadata.yield, attributes: normalizedMetadata.attributes, apuComponents: components, primary: true, locationJson: typeof entry.locationJson === "object" && entry.locationJson !== null && !Array.isArray(entry.locationJson) ? entry.locationJson as Record<string, unknown> : {} }; }) };
