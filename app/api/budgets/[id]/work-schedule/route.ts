...
import { generateWorkScheduleBase, getWorkScheduleOverviewSection, saveWorkScheduleItem, setWorkScheduleBaseline } from "@/lib/data/work-schedule";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "No se pudo establecer la linea base del cronograma" },
      { status: 400 },
    );
  }
}
