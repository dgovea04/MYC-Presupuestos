...
describe("knowledge provenance explorer", () => {
  it("returns source and review evidence links for admin inspection", async () => {
    await expect(getKnowledgeEvidenceProvenance("ke1")).resolves.toEqual(expect.objectContaining({ id: "ke1", source: expect.objectContaining({ id: "s1", label: "Review", sourceType: "MC_REVISOR" }), provenance: expect.objectContaining({ documentId: "doc1", documentVersionId: "dv1", documentVersion: 3, findingIds: ["f1"], locations: [{ sheet: "Items", range: "A1:B2" }] }) }));
  });
});
