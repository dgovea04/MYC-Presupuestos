...
export async function retrieveKnowledgeV1(input: RetrievalV1Input): Promise<RetrievalV1Result> {
  ...
  const [prices, yields, apuVersions, assertions] = await Promise.all([
    ...
    Promise.all(scopes.map((scope) => prisma.knowledgeApuVersion.findMany({ where: { scope: scope.scope, companyId: scope.companyId, name: { contains: query, mode: "insensitive" } }, include: { source: true, evidence: true }, orderBy: { createdAt: "desc" }, take: limit }))),
