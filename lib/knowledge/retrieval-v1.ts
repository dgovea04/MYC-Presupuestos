...
function enrich<T extends Record<string, unknown>>(row: T, query: string, aliases: string[] = []): T & { _retrievalScore: number; provenance: Record<string, unknown> } {
  const aliasMatch = aliases.some((alias) => normalizeKnowledgeText(alias) === query);
  const provenanceLinks = Array.isArray(row.provenanceLinks) ? row.provenanceLinks.filter((link): link is Record<string, unknown> => typeof link === "object" && link !== null) : [];
  const sourceIds = provenanceLinks.map((link) => link.sourceId).filter((id): id is string => typeof id === "string");
  const evidenceIds = provenanceLinks.map((link) => link.evidenceId).filter((id): id is string => typeof id === "string");
  return { ...row, _retrievalScore: score(String(row.name ?? row.normalizedName ?? ""), query, String(row.scope ?? "GLOBAL"), aliases), provenance: { sourceId: row.sourceId ?? sourceIds[0] ?? null, evidenceId: row.evidenceId ?? evidenceIds[0] ?? null, sourceIds, evidenceIds, sources: row.source ?? provenanceLinks.map((link) => link.source), evidences: row.evidence ?? provenanceLinks.map((link) => link.evidence), scope: row.scope ?? "GLOBAL", companyId: row.companyId ?? null, projectId: row.projectId ?? null, matchedBy: aliasMatch ? "ALIAS_EXACT" : normalizeKnowledgeText(String(row.name ?? row.normalizedName ?? "")) === query ? "NAME_EXACT" : "NAME_PARTIAL" } };
}
...
export async function retrieveKnowledgeV1(input: RetrievalV1Input): Promise<RetrievalV1Result> {
  ...
  const [items, resources] = await Promise.all([
    Promise.all(scopes.map((scope) => prisma.canonicalItem.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(input.status ? { status: input.status } : {}), OR: [{ normalizedName: { contains: query } }, { aliases: { some: { normalizedAlias: query } } }] }, include: { aliases: true, provenanceLinks: { include: { source: true, evidence: true } } }, orderBy: { updatedAt: "desc" }, take: limit }))),
    Promise.all(scopes.map((scope) => prisma.canonicalResource.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(input.status ? { status: input.status } : {}), OR: [{ normalizedName: { contains: query } }, { aliases: { some: { normalizedAlias: query } } }] }, include: { aliases: true, provenanceLinks: { include: { source: true, evidence: true } } }, orderBy: { updatedAt: "desc" }, take: limit }))),
  ]);
  ...
  const [prices, yields, apuVersions, assertions] = await Promise.all([
    Promise.all(scopes.map((scope) => prisma.priceObservation.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(scope.projectId ? { projectId: scope.projectId } : {}), resourceId: { in: resourceIds }, ...(input.status ? { status: input.status } : {}), ...(input.confidence ? { confidence: input.confidence } : {}), ...(input.regionId ? { regionId: input.regionId } : {}) }, include: { source: true, evidence: true }, orderBy: { observedAt: "desc" }, take: limit }))),
    Promise.all(scopes.map((scope) => prisma.yieldObservation.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(scope.projectId ? { projectId: scope.projectId } : {}), canonicalItemId: { in: itemIds }, ...(input.status ? { status: input.status } : {}), ...(input.confidence ? { confidence: input.confidence } : {}), ...(input.regionId ? { regionId: input.regionId } : {}) }, include: { source: true, evidence: true }, orderBy: { observedAt: "desc" }, take: limit }))),
    Promise.all(scopes.map((scope) => prisma.knowledgeApuVersion.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(scope.projectId ? { projectId: scope.projectId } : {}), name: { contains: query, mode: "insensitive" } }, include: { source: true, evidence: true }, orderBy: { createdAt: "desc" }, take: limit }))),
    Promise.all(scopes.map((scope) => prisma.knowledgeAssertion.findMany({ where: { scope: scope.scope, companyId: scope.companyId, ...(scope.projectId ? { projectId: scope.projectId } : {}), ...(input.status ? { status: input.status } : {}), ...(input.confidence ? { confidence: input.confidence } : {}), OR: [{ subjectId: { in: [...itemIds, ...resourceIds] } }, { predicate: { contains: query, mode: "insensitive" } }] }, orderBy: { updatedAt: "desc" }, take: limit }))),
  ]);
