...
export async function appendBudgetChangeEvent(input: { budgetId: string; userId: string; entityType: "BUDGET" | "BUDGET_ITEM" | "APU" | "METRADO" | "REVIEW_FINDING"; entityId: string; action: string; field: string; oldValue: string | null; newValue: string | null; source: "USER" | "SYSTEM" | "KHIPU" | "INTEGRATION"; requestId?: string; client?: Prisma.TransactionClient }): Promise<BudgetChangeRecord> {
  const ownership = await resolveBudgetOwnership(input.budgetId, input.userId);
  if (input.requestId) {
    const existing = await (input.client ?? prisma).budgetChangeEvent.findFirst({ where: { budgetId: input.budgetId, requestId: input.requestId } });
    if (existing) return serializeChangeEvent(existing as never);
  }
  const event = await (input.client ?? prisma).budgetChangeEvent.create({ data: { companyId: ownership.companyId, projectId: ownership.projectId, budgetId: input.budgetId, entityType: input.entityType, entityId: input.entityId, action: input.action, field: input.field, oldValue: input.oldValue, newValue: input.newValue, source: input.source, userId: input.userId, requestId: input.requestId }, });
