...
async function persistedMetrics(run: Row, companyId: string, client: ReviewJobClient): Promise<ReviewRunMetrics | undefined> {
  ...
  const evidence = await client.reviewEvidence.findMany({ where: { companyId, projectId, documentVersionId: { in: versionIds } } });
...
async function persistedMetricsWithoutDelta(run: Row, companyId: string, client: ReviewJobClient, budgetIds: Set<string>): Promise<ReviewRunMetrics | undefined> {
  ...
  return calculateReviewRunMetrics({ budgetIds: [...budgetIds], budgetItems: items.map((item) => ({ id: String(item.id), budgetId: String(item.budgetId) })), evidence: evidence.map((item) => ({ id: String(item.id), documentVersionId: String(item.documentVersionId) })), links: links.map((item) => ({ budgetItemId: String(item.budgetItemId), evidenceId: String(item.evidenceId) })), findings: findings.map((item) => ({ budgetItemId: typeof item.budgetItemId === "string" ? item.budgetItemId : null, status: String(item.status), findingType: String(item.findingType) })), warnings: Array.isArray(run.warningsJson) ? run.warningsJson as Array<{ code: string; message: string }> : [] });
}
