...
export interface ReviewJobClient { reviewRun: { findUnique(args: { where: Where }): Promise<Row | null>; update(args: { where: Where; data: Row }): Promise<Row>; findMany(args: { where: Where }): Promise<Row[]>; }; }
...
export async function getReviewProgress(reviewRunId: string, companyId: string, client: ReviewJobClient, options: { staleAfterMs?: number } = {}): Promise<ReviewProgress> {
  ...
  if (["DRAFT", "QUEUED", "RUNNING"].includes(status) && Date.now() - updatedAt > staleAfterMs) { status = "STALE"; await client.reviewRun.update({ where: { id: reviewRunId, companyId }, data: { status } }); }
...
export async function requestReviewCancellation(reviewRunId: string, companyId: string, client: ReviewJobClient): Promise<void> {
  const run = await client.reviewRun.findUnique({ where: { id: reviewRunId, companyId } });
  if (!run) throw new Error("Review run not found.");
  if (["DRAFT", "QUEUED", "RUNNING"].includes(String(run.status))) await client.reviewRun.update({ where: { id: reviewRunId, companyId }, data: { status: "CANCELLED", finishedAt: new Date() } });
}
