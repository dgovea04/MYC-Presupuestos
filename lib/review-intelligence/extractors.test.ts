...
describe("review document extractors", () => {
  ...
  it("warns for OOXML macros, external links, external formulas and hyperlinks without executing them", async () => {
    ...
    const bytes = await workbook.xlsx.writeBuffer();
    const marker = new TextEncoder().encode("vbaProject.bin externalLinks externalBook [Book.xlsx]Sheet1!A1 hyperlinks");
    const fixture = new Uint8Array(bytes.byteLength + marker.byteLength);
    fixture.set(new Uint8Array(bytes));
    fixture.set(marker, bytes.byteLength);
    ...
    expect(result.warnings).toEqual(expect.arrayContaining([expect.stringContaining("macro"), expect.stringContaining("extern")]));
  });

  it("uses the compatible PDF importer and states that page count may be estimated and exact location is unavailable", async () => {
    const file = new File(["%PDF-1.7\n01.01 Trazo y replanteo m2 10 2.50 25.00"], "spec.pdf", { type: "application/pdf" });
    const result = await extractDocument({ file });
    expect(result.kind).toBe("PDF");
    expect(result.items[0]?.content).toContain("Trazo y replanteo");
    expect(result.items[0]?.location).toBeUndefined();
    expect(result.warnings).toEqual(expect.arrayContaining([expect.stringContaining("conteo de páginas PDF puede ser estimado"), expect.stringContaining("ubicación exacta no está disponible")]));
  });
