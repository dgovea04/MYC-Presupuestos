...
type FindingRow = Prisma.ReviewFindingGetPayload<{
  select: {
    ...
    decisions: { select: { id: true; userId: true; resolution: true; note: true; expectedUpdatedAt: true; createdAt: true }; orderBy: { createdAt: "desc" }; take: 1 };
  }
}>;

export type FindingFilters = {
  companyId: string; projectId?: string; budgetId?: string; reviewRunId: string; page: number; pageSize: number;
  status?: FindingStatus; findingType?: Prisma.ReviewFindingWhereInput["findingType"]; severity?: string; confidence?: Prisma.ReviewFindingWhereInput["confidence"];
};
export type PaginatedFindings = { findings: Array<Record<string, unknown>>; page: number; pageSize: number; hasNextPage: boolean };
export type FindingDecisionInput = { findingId: string; companyId: string; userId: string; resolution: FindingResolution; note?: string; expectedUpdatedAt: Date; reconfirmStale?: boolean };
...
const secret = () => process.env.NEXTAUTH_SECRET ?? process.env.AUTH_SECRET ?? "mc-review-evidence-development-secret";
...
export function verifyTemporaryEvidenceToken(evidenceId: string, token: string): boolean {
  ...
  const expected = createHmac("sha256", secret()).update(`${evidenceId}.${expiresAt}`).digest("base64url");
...
export async function listFindings(filters: FindingFilters, client: Client = prisma): Promise<PaginatedFindings> {
  ...
  const where: Prisma.ReviewFindingWhereInput = { companyId: filters.companyId, projectId: run.projectId, budgetId: run.budgetId, reviewRunId: run.id, status: filters.status, findingType: filters.findingType, severity: filters.severity, confidence: filters.confidence };
...
export async function recordFindingDecision(input: FindingDecisionInput, client: Client = prisma): Promise<FindingDecisionRecord> {
  return client.$transaction(async (tx) => {
    ...
    if (current.status === "STALE" && !input.reconfirmStale) throw new Error("Finding is stale; reconfirmation required.");
    const changed = await tx.reviewFinding.updateMany({ where: { id: input.findingId, companyId: input.companyId, projectId: current.projectId, budgetId: current.budgetId, updatedAt: input.expectedUpdatedAt }, data: { status: input.resolution === "NEEDS_MORE_EVIDENCE" ? "IN_REVIEW" : input.resolution === "NOT_APPLICABLE" ? "DISMISSED" : "RESOLVED" } });
    ...
    await tx.reviewAuditEvent.create({ data: { companyId: input.companyId, projectId: current.projectId, reviewRunId: current.reviewRunId, actorUserId: input.userId, eventType: "FINDING_DECISION_RECORDED", payloadJson: { findingId: input.findingId, previousStatus: current.status, resolution: input.resolution, expectedUpdatedAt: input.expectedUpdatedAt.toISOString() } } });
