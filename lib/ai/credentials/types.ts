...
export const aiCredentialScopeSchema = z.enum(["PLATFORM", "WORKSPACE", "USER"]);
...
export const aiCredentialInputSchema = z
  ...
  .superRefine((value, context) => {
    const hasWorkspace = Boolean(value.workspaceId);
    const hasUser = Boolean(value.userId);
    const validOwner =
      (value.scope === "PLATFORM" && !hasWorkspace && !hasUser) ||
      (value.scope === "WORKSPACE" && hasWorkspace && !hasUser) ||
      (value.scope === "USER" && !hasWorkspace && hasUser);

    if (!validOwner) {
      context.addIssue({
        code: "custom",
        path: ["scope"],
        message: "El scope y su propietario no coinciden.",
      });
    }
  });
...
export function providerToCredentialProvider(provider: AiProviderId | AiCredentialProvider): AiCredentialProvider | null {
  if (provider === "agent") return "OPENROUTER";
  if (provider === "openai" || provider === "gemini" || provider === "openrouter") {
    return provider.toUpperCase() as AiCredentialProvider;
  }
  if (provider === "OPENAI" || provider === "GEMINI" || provider === "OPENROUTER") {
    return provider;
  }
