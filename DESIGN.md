...
| 3. Polynomial frame density | `polynomial formula table: Excel density frame` | `/budgets/<id>/polynomial-formula` (real route; top-level `/polynomial-formula` does not exist) | `[data-testid="polynomial-monomials-table-frame"]` carries `rounded-md` (not `rounded-2xl`) AND tracks `--excel-row-height` on the nearest `[data-view-mode="excel"]` ancestor. | **🟨 verified by code-searcher + manual smoke (auth-bounded, expected-fail)** — spec + production hook both exist (polynomial-monomials-table.tsx:198 + the table unit test in `components/budget/polynomial-monomials-table.test.tsx:60,78`), BUT `prisma/seed.ts` does NOT seed a polynomial formula for the demo general budget, so `<PolynomialFormulaEditor>` never mounts on the route. The e2e test is wrapped in `test.fail` and surfaces red. Two unblock paths: (a) add a `seedDemoPolynomialFormula(generalBudgetId)` helper to `prisma/seed.ts` that calls the existing `generatePolynomialFormulaFromBudget(budgetId)` helper from `lib/data/polynomial-formulas.ts` (it reads the budget's monomials + base indices and persists via `prisma.polynomialFormula` + `prisma.polynomialMonomial` + `prisma.polynomialMonomialComponent` per the schema at `prisma/schema.prisma#model:PolynomialFormula,PolynomialMonomial,PolynomialMonomialComponent`). Mirror the shape of `seedAgentWorkflows(prisma)` (accepts the prisma client, returns a result object, called from `main()` after the catalog seed). Or (b) drive a "Generate polynomial formula" click in the test before the assertion. Drop the `test.fail` wrapper once either lands.

*Note on drift resistance:* this unblock path uses function-name + model-name references only (no line numbers) because a future maintainer will follow these instructions literally — a drifted line number would lead them to the wrong code. The other cells' column entries (`budget-editor.tsx ~4427`, `general-budget-footer-table.tsx ~347/375`, etc.) keep their line numbers as informational references where drift is low-impact (a reader can grep the function/role name to find the right location today). |
