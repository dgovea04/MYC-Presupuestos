...
#### Work-ready-when-DB-seeded checklist

**Scope:** this checklist tracks cells #2-#5 + the spec-level "Excel mode applied" assertion (5 cells total). Cell #1 is **NOT** work-ready-when-DB-seeded — it is work-ready-when-`budget-editor.tsx`-ships-`data-spreadsheet-active`, which is an independent production-code dependency. Cell #1's gating is documented inline in the lock-in cell table above.

These 5 cells flip from "verified by code-searcher + manual smoke" to "verified by automated e2e in CI" the first time `npm run test:e2e` runs end-to-end against a healthy `localhost:3000` + a DB with the seeded demo user (`demo@mycpresupuestos.pe` / `Demo12345`, `emailVerifiedAt` set). Until that run lands, the cells above carry the manual-smoke label.

| Checklist cell | What needs to land |
|---|---|
| `data-view-mode="excel"` auto-applies post sign-in | Spec `beforeEach` assertion passes when the dev server is reachable and the demo user is seeded. |
| APU subpartida dialog inheritance | Cell #2 — Radix dialog + `apu-editor-sheet.tsx` inheritance. |
| Polynomial frame density | Cell #3 — `polynomial-monomials-table.tsx` + nearest `[data-view-mode="excel"]` ancestor. |
| General budget footer density | Cell #4 — `general-budget-footer-table.tsx` arbitrary-value class + CSS variable. |
| CompactRowActions per-row menu | Cell #5 — `/resources` + `/partidas` triggers + role=menu surfaces. |

**Flip-to-green procedure** (after the first successful `npm run test:e2e` run against a seeded DB):

1. Re-run `npm run test:e2e` and confirm `6 passed, 1 expected-fail` on stdout.
2. In the lock-in cell table above, replace each of cells #2-#5's "verified by code-searcher + manual smoke (auth-bounded)" label with "verified by automated e2e in CI".
3. Replace cells #2-#5's "full `npm run test:e2e` end-to-end run is work-ready-when-DB-seeded" suffix with a one-line CI artifact reference (PR number + run URL).
4. Delete this checklist sub-section (its job is done once automated CI backs the cells).

Report-mode expectations: the suite is **designed to print 6 passed, 1 expected-fail IF** (a) `localhost:3000` is reachable, (b) the demo user is seeded, (c) `/api/dev/set-view-mode` returns 200 for `viewMode=excel`, AND (d) all 5 lock-in cells above pass their assertions plus the dev-shortcut assertion. Lock-in cells #2-#5 have **not yet been run end-to-end in CI** — they are currently verified by code-search + manual smoke (auth-bounded). The reported `expected-fail` is the budget-editor Ctrl+D contract (lock-in cell #1) that surfaces as red only because the production hook is intentionally not yet wired. CI integration should mark the suite as passing on that pattern once the work-ready-when-DB-seeded checklist below is flipped green.
